...
> **Status, late 2026-09-08:** every checkpoint below except the optional CP-D5 is built, tested, pushed to `main` and deployed to the dev project: CP-D0–D4, CP-O1–O2, CP-N0–N3, CP-N4's iPhone files, and Stage 13's CP-E1–E4 (§11). The directory reads the live littlebluecart.com (route C, §2) and the link works against it. None is ticked yet: each waits for your tap-through, and the whole directory stage waits for the Cloudways staging site and the three secrets (§2). The boxes, pass lines and journeys live in `Planning/checkpoints.md` (Stages 10–12) and `Planning/manual-test.md` (J12–J15). One thing to know before anything else: **the next `deploy-dev` asks for the three WordPress secrets and will not finish until they exist**, so do §2's "First" and "Secrets" steps before deploying anything, including a Stage 8/9 fix.

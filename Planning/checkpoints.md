...
- [~] **0.9 `npm run touch-products`** *(written; blocked until the Shopify app has scopes, see below)* — `functions/scripts/touch-products.mjs`: a no-op `productUpdate` on every dev-store product (`--vendor "Name"`, `--limit N`) so `products/update` fires through the real webhook path and fills the `catalog` mirror. Prints "touched N; catalog now has M documents". Doubles as the end-to-end webhook test.
...
**Grace does:** `scripts\doctor.ps1` (fix reds, rerun) → `cd functions; npm run touch-products` → `scripts\run-live.ps1` → "Continue as a guest" → search for the first word of a product you know is on the dev store.
...
- [ ] **CP-A2 An existing Shopify customer signs in and sees their orders.**
...
- [ ] **CP-A3 Seller claim code → seller surfaces appear without a restart.**
...
- [ ] **CP-A4 The seller's existing products appear** (Phase 3b, tightened).
...
- [ ] **CP-A5 Shipturtle probe, then automatic vendor detection by email.**

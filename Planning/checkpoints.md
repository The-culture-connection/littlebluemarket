...
- [ ] **CP-P6 Move production to the real shop and rotate every credential.** *Status 2026-09-08, late:* steps 1–6 done except two pieces. Production is on the **real shop** with the rotated Shopify client secret, Storefront token and littlebluecart.com credentials (seven secrets, new versions); functions redeployed; the six webhooks on the real shop confirmed; the doctor reads `env params … (the real shop)`. **Open:** (a) **the new Shipturtle Order key does not work for the roster.** Grace generated a new key (Shipturtle user 876806) on 2026-09-08; it answers `/api/v1/me` and `/api/v1/orders`, but `/api/v1/users` (the vendor roster that links sellers automatically) never returns for it, even with a 45 s timeout, while the previous key (user 874318) returns the 17 users in 4 s. Production was rolled back to the previous key the same evening. **Do not delete the previous key in Shipturtle.** To retry: Shipturtle → Settings → API → check the new key's user has the same role/permissions as the old one (or ask support why `GET /api/v1/users` hangs for user 876806), then `prod-secrets.ps1 -Only SHIPTURTLE_API_KEY` + `deploy-prod.ps1`; the doctor's `shipturtle` line (or `npm run probe:shipturtle:prod`) says whether the roster answers. The script keeps only the token if a note follows it on the line; (b) the real shop's app still lacks `write_products, write_inventory, write_publications, write_fulfillments` (step 1), so sellers cannot add products on production yet; (c) `SHOPIFY_LOCATION_ID` is blank; (d) the dev project still holds the OLD littlebluecart.com credentials (the note at the end of step 3). Do these in order; each is a few minutes.

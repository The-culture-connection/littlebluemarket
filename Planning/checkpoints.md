...
- [ ] **CP-P6 Move production to the real shop and rotate every credential.** *Status 2026-09-08, late:* steps 1–6 done except two pieces. Production is on the **real shop** with the rotated Shopify client secret, Storefront token and littlebluecart.com credentials (seven secrets, new versions); functions redeployed; the six webhooks on the real shop confirmed; the doctor reads `env params … (the real shop)`. **Open:** (a) **Shipturtle's vendor roster does not answer for the real account.** The two keys turned out to be two Shipturtle companies: the old key is company 1092564 "little_blue_market_devtestingshop" (Grace's own login, made 2026-09-05 for the dev shop, 17 users, 8 orders); the new key is company 1066319 **"Little Blue Market"** (contact Kate & Erin, made April 2026, 1,095 orders), i.e. the real merchant account, generated by a Shipturtle-staff user named "littlebluecart". Production now runs on the real account's key. Its `/api/v1/me`, `/api/v1/orders` and `/api/v1/companies/{id}` answer in 1–3 s, but `GET /api/v1/users` (the roster automatic seller linking matches against) ends in a **504 from Shipturtle's own gateway after 120 s**, with or without paging or filter parameters; `/api/v1/vendors` is a 500. So the functions now give the roster 12 s and remember a failure for 10 min (`_internal/shipturtleRoster.failedAt`); seller linking on production falls back to the merchant's `vendorMappings` document and claim codes until Shipturtle fixes it. **Ask Shipturtle support** (team@shipturtle.com): "GET https://api-v2.shipturtle.com/api/v1/users for company 1066319 returns 504 after 120 s; company 1092564 answers in 4 s. Is there a paged or filtered form, or a per-email lookup?" When it answers, nothing needs redeploying: the next sign-in after the 10-minute memory expires uses it. The script keeps only the token if a quote or a note surrounds it on the line; (b) the real shop's app still lacks `write_products, write_inventory, write_publications, write_fulfillments` (step 1), so sellers cannot add products on production yet; (c) `SHOPIFY_LOCATION_ID` is blank; (d) the dev project still holds the OLD littlebluecart.com credentials (the note at the end of step 3). Do these in order; each is a few minutes.
...
**Functions:** `functions/src/{index,config,linking,sellers,vendors,catalog,orders,cart,fulfillment,shipturtle}.ts` · `functions/src/shopify/{token,storefront}.ts` · **Stages 10–12:** `functions/src/{wordpress,directory,push}.ts` · `functions/scripts/wp-probe.mjs` · `functions/src/{directory_products,buyer_index,notifications}.ts` · `admin-web/` (the Railway announcement site; static, no server logic) · `functions/scripts/{register-webhooks,seed}.mjs` · `functions/test/*` · `firebase/{firestore.rules,storage.rules,firestore.indexes.json}` · `.firebaserc` · `functions/.env.little-blue-610e5`.

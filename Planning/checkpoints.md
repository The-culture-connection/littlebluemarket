...
| Stage 12 — Push notifications: announcements, forums, shoutouts, reviews, new products, iPhone, ask at start-up, admin website | 🟡 CP-N0 to CP-N3 built 2026-09-08 (Android push end to end); CP-N4 iPhone pre-wired 2026-09-08, the rest needs a Mac (manual-test J15 step 17); CP-N5 ask-at-start-up and CP-N6 admin website (Railway) built 2026-09-08 (J15 steps 19–20) |
| Stage 13 — Directory businesses as sellers: listing fills the profile, website-link products, photo strip, apply row, "Did you buy it?" | 🟡 CP-E1 to CP-E5 built 2026-09-08, ready for Grace to test (manual-test J13 steps 8–14) |
| Stage 14 — Polish: floating bug button with screenshots, splash artwork, tappable bio links, first-time tour | 🟡 CP-F1 to CP-F4 built 2026-09-08, ready for Grace to test (manual-test J16) |
| Stage 15 — Production cutover: `flutter run` is the production app, prod Firebase project wired, secrets set, first deploy | 🟡 CP-P1 and CP-P2 done 2026-09-08; CP-P3 deploy attempted the same evening (see its status note); CP-P4/P5 are Grace's (manual-test J17) |
...
- [x] **CP-P1 Draft → Under review.**
  ...
  **Stock goes to the online-fulfilment location (learned 2026-09-05):** the store has several locations and the API lists a secondary one first; opening stock written there read "sold out" at checkout while the admin showed plenty. `SHOPIFY_LOCATION_ID` in `functions/.env.little-blue-610e5` now pins the shop's main location (121092407456 on the dev store); without it the backend asks the store (needs `read_locations`) or follows where existing stock lives. `npm run move-stock -- --vendor cc --to <location>` moves a vendor's stock; it was run for the seller's six variants.
...
**The feed fills itself (added 2026-09-05).** A product that is active on the store and attributed to a seller posts itself to the feed as that seller, once (post id `listing_<productId>`; a re-mirror updates it). Products whose vendor nobody has claimed have no author and post nothing, so an unclaimed vendor's products live in search and collections only until someone claims that vendor. `npm run touch-products` re-mirrors the whole catalog and is how the feed was seeded on the dev store.
...
- [x] **CP-S1 Becoming a seller, three doors on one page.** Edit profile → **Sell with us**: *Check my seller status* (a confirmed email on the Shipturtle roster is granted selling as the vendor string its company's products carry, on the spot); *Apply on our website* (the store's Become a vendor page, from `REGISTRATION_URL` in the env, dev and prod separately; Grace approves in Shipturtle; the applicant taps Check again); *I have a claim code*. The in-app application queue built earlier was removed in favour of this, at Grace's request (2026-09-05).
...
- [x] **CP-S6 Shipping, simplified (2026-09-05), then removed (2026-09-08).** Grace handles shipping entirely in Shipturtle, so the Packages screen, the profile's shipping icon, the Packages rows and every "on its way" line are gone; sellers keep one Edit profile row, **Orders & shipping**, that opens Shipturtle. The backend still records tracking from Shipturtle every 15 minutes (it drives "Received" and the review prompt); nothing shows it as a screen.

**Also fixed later on 2026-09-05 (Grace's second round):** *Check my seller status* now asks Shipturtle fresh (the 15-minute roster and vendor caches are dropped for that tap) and takes the vendor string from the company's **Brand name**, so a vendor approved a minute ago with no products yet is granted. The avatar's **Change photo** works, and every photo picker (avatar, shoutout, product) asks Take a photo / Choose from your photos first, because an empty gallery looked like a broken picker. Near me asks Android's own location manager on the phone (the fused provider on an emulator never sees the fix it was given), reuses the last known fix, treats a timeout as "no fix" instead of an error, and offers **Add my city**. Hashtag search over reviews needed a collection-group read rule for `reviews`. Then it turned out nothing carried a hashtag at all: only Shopify tags typed with a '#' were kept, and nobody types those, so the vocabulary was empty. Every store tag is now a hashtag (`#FeministGift`), every `#tag` typed into a post becomes one of its tags, the hashtag counter is keyed case-insensitively, and a search for `#plasticfree` finds `#PlasticFree`. The profile-setup screen's Add a photo was inert; it now picks and uploads with the profile.
...
- [ ] **CP-D0 A development WordPress + WooCommerce.** *Claude built:* `WP_BASE_URL`, `WP_APP_USER` and `DIRECTORY_ADD_LISTING_URL` in the env; a doctor line `wordpress` that reaches `<WP_BASE_URL>/wp-json/` and reads one directory listing, and FAILs when `WP_BASE_URL` is the live littlebluecart.com unless `WP_LIVE_OK=yes`.
  **Route C, the one in use (Grace's decision, 2026-09-08): read the LIVE site from dev.** littlebluecart.com's host blocks plugin installs and no Cloudways login is at hand, so there is no staging copy. The app never writes to WordPress and the WooCommerce key is Read, so the dev project reads the live site with `WP_LIVE_OK=yes` in the env (already set, with `WP_BASE_URL=https://littlebluecart.com`). The doctor shows a yellow `wordpress (live)` line on every run as the reminder. **What this means for testing:** use accounts that already exist on littlebluecart.com wherever possible; any test user, order or listing you add there is real, so delete it when the checkpoint passes. Parts 2–5 of the walkthrough (Application Password, WooCommerce Read key, the three `secrets:dev` prompts, `WP_APP_USER`) are done on the live wp-admin. Routes A (Cloudways staging) and B (WP Staging plugin) below stay documented for the day a copy becomes possible; switching is one env line.
  **Grace does (once). Route B, from wp-admin only (chosen 2026-09-08: Grace has no Cloudways login):** 1. littlebluecart.com `/wp-admin` → Plugins → Add New → search **WP Staging** → Install → Activate. 2. **WP Staging** in the left menu → **Create Staging Site** → name `dev` → keep every table and folder ticked → **Start Cloning** (10–30 minutes; leave the tab open). 3. When it says done, the copy lives at `https://littlebluecart.com/dev/` and its wp-admin at `https://littlebluecart.com/dev/wp-admin/` (same username and password as live; visitors who are not signed in see a login box, which is fine). 4. In the **copy's** wp-admin: if WooCommerce asks, choose **This is a temporary / staging site**; WooCommerce → Settings → Payments → every live method off; Settings → Reading → tick **Discourage search engines**; Plugins → Add New → install and activate **Disable Emails**. 5. Put the copy's address in `REPO\functions\.env.little-blue-610e5`: `WP_BASE_URL=https://littlebluecart.com/dev` and `DIRECTORY_ADD_LISTING_URL=https://littlebluecart.com/dev/add-directory-listing/`, then `scripts\doctor.ps1`. *(Route A, if a Cloudways login ever turns up: Cloudways → the app → Staging Management → Add Staging Application; the URL is then `https://wordpress-…cloudwaysapps.com` with Password Protection off.)*
  **Pass:** `https://littlebluecart.com/dev/` opens and looks like littlebluecart.com (a login box for visitors is expected); after CP-D1 the doctor line reads `PASS wordpress staging reachable · <site name> · littlebluecart.com/dev (a copy in a folder under the live domain) · visitors are gated, the app password gets through (fine for staging)`. Before CP-D1 it says the site gates visitors and to finish CP-D1; that is the expected order.
  **If it fails:** `FAIL wordpress WP_BASE_URL is the LIVE site` → the env line is missing the `/dev` folder. WP Staging stops part-way → run **Create Staging Site** again; it resumes. `http` only → the copy inherits the live site's https, so this should not happen; paste the line. Anything else → paste the doctor block.
- [ ] **CP-D1 Credentials, WordPress client, doctor.** *Claude built (2026-09-08):* `WP_APP_PASSWORD`, `WC_CONSUMER_KEY`, `WC_CONSUMER_SECRET` secrets; `functions/src/wordpress.ts` (the only place that talks to WordPress); `wordpress` and `woocommerce` rows in the backend health check; `npm run wp:probe -- --email <addr>`.
...
- [ ] **CP-D2 Link your directory account and see website orders.** *Claude built (2026-09-08):* `directoryLinkMe` / `directoryRefresh` (email from the verified token only), the server-only `directory/{uid}` doc and `users/{uid}/directoryOrders`, the **Little Blue Cart directory** screen (Edit profile → Little Blue Cart directory), and a silent once-per-launch link so directory customers get their orders without choosing a role.
...
- [ ] **CP-O1 The "Are you…" screen and the door that survives sign-up.** *Claude built (2026-09-08):* `/orient` with the seven doors; `?intent=` carried through `/signin` → `/verify` → `/setup`; landing: new here → Market; bought on littlebluecart.com → Market as well (changed 2026-09-08: the site has no customer accounts, so the door is a label; website orders still arrive through the silent link by billing email); bought on the Market → your profile's Bought tab; listed in the directory → Directory; sell on the Market → Sell with us (checking by itself); want to list a business → Directory, which opens Add Your Business; want to sell on the Market → Sell with us, which opens Apply on our website. The dead "I'm here to sell" checkbox is removed.
...
- [ ] **CP-O2 The landing screens finish the job.** *Claude built (2026-09-08):* Sell with us and Directory run their check by themselves when arrived at from a door, once the email is confirmed (the banner's **I've confirmed it** triggers it).
...
- [ ] **CP-N4 iPhone.** *Claude pre-wired (2026-09-08):* `ios/Runner/Runner.entitlements`, background mode, project settings; a doctor MANUAL line with the steps; `scripts/run-live-mac.sh`. Needs a Mac with Xcode and a real iPhone: **the whole Mac setup, step by step, is `Planning/mac-iphone.md`.**
...
- [ ] **CP-N5 Ask for notifications at start-up.** *Claude built (2026-09-08, Grace's call):* the app asks the phone for notification permission the moment it opens, before anyone signs in, and only while the phone has never been asked; a refusal is respected afterwards (Edit profile → Notifications → **Allow notifications** is the way back in, and on Android that opens App info once the phone has said no).
  **Grace does:** uninstall the app from the emulator (so the phone forgets its answer), `run-live.ps1` again.
  **Pass:** Android's "Allow Little Blue Market to send you notifications?" prompt appears over the welcome screen; Allow; sign in; Edit profile → Notifications already reads "Allowed on this phone." Open the app a second time: no prompt.
  **If it fails:** no prompt → the emulator is below Android 13 (older Androids never ask), or the app was not uninstalled first. Prompt every launch → Copy for Claude.
- [ ] **CP-N6 An admin website for announcements, on Railway.** *Claude built (2026-09-08):* `admin-web/`, a one-page site (sign in with the app account, admin claim required, title / message / who / a tap opens, **Send**, recent announcements) that calls the same `adminSendAnnouncement` function the in-app Admin screen calls; `admin-web/README.md` has the Railway steps. The Firebase **web app** it needs (`LBM admin console`, `1:125444581085:web:c231a1e76f44f3b0505d34`) is registered on the dev project and its public config is in `admin-web/public/firebase-config.js`.
  **Grace does:** railway.app → New Project → Deploy from GitHub repo → this repo → Settings → Root Directory `little_blue_market/admin-web` → Networking → Generate Domain → copy it. Firebase console → Authentication → Settings → **Authorized domains** → Add domain → paste it. Open the domain → sign in as `grace-s@…` → title "Hello from the website", Everyone → Send.
  **Pass:** the page says "Sent to everyone.", the push lands on the signed-in test phone, the announcement appears in the page's Recent list and in the app's bell. Signing in as `+buyer1` shows only "Admins only".
  **If it fails:** "not on the Firebase Authentication authorized-domains list" → the Add domain step. "Admins only" for your account → Claim admin in the app's Diagnostics, sign out and in on the website. Railway build red → Root Directory is wrong (it must be the `admin-web` folder). To try it on your computer first: `cd little_blue_market\admin-web`, `npm install`, `npm start`, open http://localhost:3000.
...
- [ ] **CP-E2 Products with a Buy button that opens the website.** *Claude built (2026-09-08):* `directoryProducts/{id}` (title, price optional, description, photos, buy link prefilled with the listing website) written only through `directoryProductSave` / `directoryProductDelete`, which refuse anyone not joined to the directory; a feed post per product; the product page and the feed card show **Buy on their website** instead of Add to cart; an **Add a product** form for directory sellers (photos, title, price, description, link).
...
- [ ] **CP-E5 "Did you buy it?" for website purchases.** *Claude built (2026-09-08, Grace's call):* a purchase made on a directory business's own website is invisible to the app, so when a signed-in member taps **Buy on their website** and comes back, the app asks **Did you buy it?** — **Yes, I bought it** calls `directoryPurchaseReport`, which puts the product under Bought on their profile (marked self-reported, never a sales figure), lets them review it, and counts them as a buyer of that shop for "new from a shop you bought from" pushes. **Not this time** does nothing. Guests are never asked.
  **Grace does:** as `+buyer1`: open a website-link product → **Buy on their website** → the browser opens → come back to the app.
  **Pass:** the "Did you buy it?" dialog is there; **Yes, I bought it** → "Added under Bought on your profile."; You → Bought shows the product (tap it → Write a review works); Firestore `users/<uid>/purchases/website_<id>_<time>` has `selfReported: true`. Buying again asks again; **Not this time** leaves Bought unchanged.
  **If it fails:** no dialog → the account was a guest (sign in) or the browser never opened (a "Could not open …" toast). Anything red → Copy for Claude.

### Stage 14 — Polish (added 2026-09-08, Grace's asks)

- [ ] **CP-F1 The floating bug button.** *Claude built (2026-09-08):* a small round button at the bottom right of every screen except the welcome artwork. A tap photographs the screen as it is, then opens a sheet: **Something is broken** / **A critique or an idea**, a few words, the picture (with a switch to leave it out), **Send**. Guests can send too. Everything lands in `feedback/` with the screen's route, the phone type and who sent it; the screenshot goes to Storage `feedback/{uid}/`. Both admin surfaces list them: in the app, Edit profile → **Admin** → **Bugs and critiques** (tap the picture to see it large, **Mark done** / **Reopen**, **Show done**); on the admin website the same list with the same buttons. Nothing can be deleted from either.
  **Grace does:** as `+buyer1`: open any product → tap the bug button → Something is broken → "The price looks wrong" → Send. As `grace-s@…`: Edit profile → Admin → Bugs and critiques.
  **Pass:** the sheet shows a thumbnail of the product page; after Send a toast "Sent. Thank you for telling us."; the Admin screen lists it with the thumbnail, "Bug", the sender's name, the route `/market/product/…`; tapping the thumbnail shows the full screenshot; Mark done moves it under Show done. The admin website shows the same row. Firestore `feedback/<id>` has `screenshotUrl`.
  **If it fails:** no button → you are on the welcome screen (it hides there) or the keyboard is up. Sent but no picture → the phone refused the capture (the note still arrives). "Admins only" in the list → claim admin. Anything red → Copy for Claude.
- [ ] **CP-F2 The splash screen.** *Claude built (2026-09-08), reworked later the same night:* the splash is now the app icon centred on its blue, on every phone and at every moment. Grace's `Body (2).png` is the one source; `dart run tool/make_brand_images.dart` writes the launcher icon, the adaptive foreground, the Flutter splash image (`assets/images/splash-icon.png`) and the native launch images (Android `drawable-*/launch_icon.png` at 200dp, iOS `LaunchImagex/2x/3x` at 200pt). Android `launch_background.xml` and iOS `LaunchScreen.storyboard` centre that icon on `#70A0D1`; the app then holds the same icon at the same size over its first frame (`SplashOverlay`) before fading to the welcome screen. The old full-bleed `splash-body.png` is gone. **App icon:** `assets/icon/app-icon.png` is the same `Body (2).png`; `dart run flutter_launcher_icons` writes every Android and iOS size, with an adaptive icon (blue background, the cart padded into the safe zone in `assets/icon/app-icon-fg.png`). Regenerate with that command after changing the image; then `git checkout -- ios/Runner.xcodeproj/project.pbxproj`, because the generator edits the wrong Xcode setting.
  **Grace does:** force-close the app and open it.
  **Pass:** blue at once, the cart-and-wordmark artwork for about a second and a half, then the welcome screen. No white flash. On Android 12+ the phone draws the app icon on the blue first; that is Android's own splash and cannot be replaced.
  **If it fails:** a white flash before the blue → the phone cached the old launch theme; uninstall and reinstall. Artwork stretched → it is drawn to cover the screen; on a very wide tablet the edges crop, by design.
- [ ] **CP-F3 Tappable links in the bio.** *Claude built (2026-09-08):* any web address in a bio (`https://…` or `www.…`) is underlined and opens in the browser, on your own profile and on anyone's. Edit profile → Bio says so and has room for four lines. Nothing changes in what is stored, so the directory-filled bios (which carry the website) get links for free.
  **Grace does:** Edit profile → Bio → add "Shop at www.littlebluecart.com" → Save → You tab → tap the address.
  **Pass:** the address is underlined and the browser opens it; the rest of the bio is plain; the same on your public profile seen from another account.
- [ ] **CP-F5 Reporting members, and bans (Grace's ask, 2026-09-09).** *Claude built:* the **…** button on anyone's profile and on any post opens a menu with **Report**; the report sheet asks for a reason (Spam or scam · Harassment or hate · Inappropriate content · Fake shop or counterfeit · Something else) and a few words, and files `reports/{id}` (members only; a guest is sent to make a profile; nobody can report themselves). Admins see them in two places: the app's Admin screen → **Reports about members**, and the admin website's **Reports about members** card, open first, each with a **Resolve** button and a **Ban** button behind a confirm. A ban is the callable `adminBanUser`: the person's sign-in is disabled and their sessions revoked (they are out on their next request; a later sign-in says "This account has been suspended by Little Blue Market"), their profile is marked, their seller grant revoked, their posts removed, and every open report about them closed as Banned. **Unban** (`adminUnbanUser`) restores the sign-in; the posts do not come back. The person reported is never told who reported them.
- [ ] **CP-F6 Follow someone for a push when they post (Grace's ask, 2026-09-09).** *Claude built:* a **Notify me** button next to Message on every profile that is not your own (a guest is sent to make a profile). One tap follows (`users/{you}/following/{them}`, yours alone), the button reads **Notifying you**, one more tap unfollows. The function `onFollowWritten` mirrors that into `users/{them}/subscribers/{you}`, which no phone can read, so nobody can list someone's followers. When a followed person creates a post, `onPostWritten` sends every subscriber (capped at 500 per post) a push "<name> posted" with the first line of the post, plus a bell row "<name> posted something new" that opens the post. Edits and like counters do not re-send. Edit profile → Notifications gains **Posts from people you follow** (on by default). *Grace does:* as `+buyer1` open `+seller1`'s profile → **Notify me** → background the app; as `+seller1` post anything. *Pass:* buyer's phone gets "<seller> posted", tap opens the post; the bell has the row; tapping **Notifying you** and posting again sends nothing. *If it fails:* no push but the bell row is there → the N0 device check for that account; nothing at all → `firebase functions:log --only onPostWritten,onFollowWritten --project prod`.
- [ ] **CP-F7 Hardening pass (code review, 2026-09-09).** *Claude built:* the shared send bar (DMs, chatroom, thread replies, post comments) now waits for the write: the button is off and the text stays until it lands, a refused or offline send puts the text back with the reason, and a double tap is one message; guests no longer see the chatroom bar. Comment likes work (the old lookup was a query Firestore refuses). A DM thread no longer freezes at its first 200 messages. A new forum counts its creator once, not twice. A thread's badge clears for messages that arrive while it is open, and on the storefront path too. Rules: every client-written message, comment, thread and post must carry the server clock as createdAt, text is a string under a cap (2,000 characters; thread titles 200, bodies 5,000), a phone may only create listing, cart and shoutout posts, and a conversation summary can only clear your own unread count or add one to the other side with a message. Counter triggers use update (a deleted post no longer comes back as a ghost when its comments are cleaned up), and the follower fan-out runs in parallel batches. The rules suite now runs in scripts/test-all (needs Java for the emulator) with new cases for all of the above; a Composer widget test locks in the send-bar behaviour. *Grace does:* nothing new to tap; the next full pass of J16 covers it. *Pass:* sending with airplane mode on keeps the text and shows "No connection"; the same tap twice sends once.
- [ ] **CP-W1 The app as a website on Railway (Grace's ask, 2026-09-09).** *Claude built:* a Flutter web build of the same app, served by `Dockerfile.web` (Flutter build stage, then the Node server in `web-server/`). In a desktop browser the app sits inside a 430-point phone frame; on a phone's browser it fills the screen. Production Firebase (the web app already registered for the admin console); push is off in the browser, everything else is live. `dart:io` is gone from the app (`lib/platform/`), which is what had kept it off the web. *Grace does:* Railway → New → Deploy from GitHub repo → this repo, root `/`, Variables `RAILWAY_DOCKERFILE_PATH=Dockerfile.web` → Deploy → Settings → Networking → Generate Domain; then Firebase console (prod) → Authentication → Settings → Authorized domains → Add that domain. *Pass:* the domain opens the welcome screen inside a phone frame; Sign in works; the feed loads real products; Buy opens the store's checkout in a new tab. *If it fails:* "auth/unauthorized-domain" in the browser → the Authorized domains step; a blank page → the Railway build log (the Flutter stage must end with `Compiling lib/main.dart for the Web... done`).
- [ ] **CP-A1 Android App Bundle for the Play Store (Grace's ask, 2026-09-10).** *Claude built:* an upload keystore in `PARENT\android-signing\` (outside git, with a README.txt on how to recreate it), `android\key.properties` wired into the Gradle release signing with a debug fallback, and `scripts\build-android-release.ps1`, which builds `flutter build appbundle --release`, copies the .aab into `android-signing\releases\` named by version, and prints the upload certificate fingerprints. *Grace does:* Play Console → create the app (package `com.littleblue.market`) → Production or Internal testing → Create new release → upload the .aab from `android-signing\releases\` → fill the store listing, content rating and data safety (the app collects email, name, photos, approximate location for Near me, and purchase history) → submit. Keep `android-signing\` backed up. *Pass:* the Play Console accepts the bundle with no signing error and shows version 0.3.1 (3). *If it fails:* "version code 3 has already been used" → raise the `+N` in pubspec.yaml and rebuild; a signing mismatch on a later upload → the keystore changed; restore it from backup or request an upload-key reset in Play Console → Setup → App signing.
- [ ] **CP-A2 First Play Store upload (Grace's ask, 2026-09-11).** *Claude built:* `Planning/play-store.md`, the walkthrough: the two signing keys and why their fingerprints differ (the recurring "different SHA" trouble), the build command, the first-upload steps, the App content answers written for this app (privacy policy URL, a test account for reviewers, 18+, no ads, and the data safety list: name, email, photos, precise location for Near me, purchase history, messages), and a table of what each Play rejection means. `android-signingREADME.txt` gains a place to record Google's app signing fingerprints after the first upload. Built and verified `little-blue-market-0.3.2-build4.aab`: signed with the same upload key as build 3 (SHA1 0F:7D:DA:E5…), versionCode 4, versionName 0.3.2, package `com.littleblue.market`, targetSdk 36, and the production Firebase project compiled in with no trace of the dev one. *Grace does:* Play Console → Create app → Production → Create new release → accept Play App Signing → upload the .aab → finish App content → roll out. *Pass:* Play accepts the bundle with no signing error and shows 0.3.2 (4). *If it fails:* the table in play-store.md §6.
- [ ] **CP-A3 Delete my account or data (Grace's ask, 2026-09-11).** *Claude built:* a public page at `/delete-account` inside the app, so it works on a phone and at `https://lbm-web-production.up.railway.app/delete-account` in a browser. Two answers, which is what both app stores ask about: delete the account and everything with it, or delete the data and keep the account. It works signed out (the address is typed) and signed in (the address comes from the account and is locked, so a request can never name somebody else). The page files through an unauthenticated callable, `requestAccountDeletion`; no client writes `deletionRequests` directly, and only admins read or close them. One open request per address. The Admin screen gains **Delete my account requests** with Mark done and, behind a confirm, Delete now, which runs `adminDeleteAccount`: posts, profile and every subcollection go, the sign-in goes unless they asked to keep it, the seller grant is revoked, and orders stay as financial records, which the page says plainly. Edit profile links to it beneath the policy rows. *Grace does:* paste the address above into Play Console → App content → Data deletion, and into App Store Connect's account deletion field; then open it once signed out and send a test request. *Pass:* the page loads without signing in, a request appears under Delete my account requests in the Admin screen, and Mark done closes it.
- [ ] **CP-B1 Two tester bugs (Grace's report, 2026-09-10).** *Claude found:* (1) **Bug reports failed for everyone but an admin.** The app uploads the screenshot then asks Storage for its download URL to write onto the document, but the Storage rule allowed *read* to admins only, so `getDownloadURL` was refused and the whole report was lost. The sender now reads their own screenshot, and a failed upload no longer loses the note (the words are the report). (2) **A profile's hashtags were never searched.** They are stored on `users/{uid}.tags` and shown as tappable chips, but the live search only prefix-matched seller handles, so tapping a chip found nothing; the demo backend had always matched them, which is how the contract was meant to read. Search now also reads profiles by hashtag, through a new lowercase mirror `tagsLower` kept true by `onUserWritten` (the directory sync writes tags too) plus the tags as typed for older profiles; the results section is now "N people" rather than "N sellers", because a directory business carrying an initiative hashtag is often not a Market seller. Staff tools gains **Reindex hashtags** (`adminBackfillProfileTags`) for profiles saved before the mirror existed. *Also found:* the TestFlight build 0.3.0 (2) was archived while the committed Firebase config pointed at the **dev** project (commit 4345f96 on 2026-09-09 put it back; the web work restored prod on 2026-09-10), so those testers' accounts, posts and bug reports all landed in `little-blue-610e5` and never appeared in the admin dashboard. *Grace does:* pull main, archive a fresh TestFlight build, and re-run the reporting test with a non-admin tester; then Staff tools → **Reindex hashtags** once. *Pass:* a tester's bug report appears in the admin dashboard with its screenshot; tapping an initiative hashtag on a profile lists the people who carry it. *If it fails:* a report with no screenshot but the words intact → the Storage upload failed for another reason, and the dev strip carries it; an empty hashtag search after the reindex → check the profile actually carries the tag (Edit profile).
  **Grace does:** as `+buyer1`: open `+seller1`'s profile → **…** → Report → Spam or scam → Send. Open one of their posts → **…** → Report this post → Something else → type a sentence → Send. As `grace-s@…`: Edit profile → Admin → Reports about members.
  **Pass:** both reports are listed open, with the reason, the reporter and (for the post) an **Open the post** button; **Resolve** on the first moves it under Show closed; **Ban @seller1** → confirm → both reports read Banned, `+seller1`'s posts are gone from the feed, and signing in as `+seller1` says the account is suspended; **Unban** lets them sign in again. The admin website shows the same rows and buttons.
  **If it fails:** the … button does nothing → an old build. "Admins only" on Ban → the account has no admin claim. Copy for Claude for anything red.
- [ ] **CP-F4 The first-time tour.** *Claude built (2026-09-08):* right after a profile is created, once the landing screen is up, a five-page card: the Market, search and Near me, the community, your profile, and the bug button. **Next** / **Done** / **Skip**, page dots. Shown at that one moment only: the phone remembers it (`Tips.firstTour`), so a second account on the same phone, or a sign-out and sign-in, never sees it again. Guests never see it. It is not shown for existing accounts that only sign in.
  **Grace does:** uninstall and reinstall (so the phone forgets), Create a Profile with a fresh `+tour1@…` address → confirm → handle → Create a profile.
  **Pass:** the tour card appears over the Market feed; Next walks the five pages; Done closes it; killing and reopening the app shows nothing; signing out and creating another profile on the same phone shows nothing.
  **If it fails:** never appears → the door landed outside the tabs (it waits for the tab shell) or the phone had seen it; reinstall. Appears twice → Copy for Claude.

### Stage 15 — Production cutover (started 2026-09-08, Grace's call)

**The two projects.** `little-blue-cart-prod` is production (`prod` alias); `little-blue-610e5` stays dev (`dev`). The real shop is `little-blue-cart-dev.myshopify.com` (littlebluemarket.com; the "dev" in its Shopify name is history). The live directory is the same littlebluecart.com either way.

**What a plain `flutter run` now is:** the production app. It talks to `little-blue-cart-prod` (the committed `google-services.json` and `lib/firebase_options.dart`), shows no corner badge, no error strip, no raw causes, no Diagnostics row: people see only the friendly copy ("That email and password do not match", "Confirm your email first"). Developer surfaces come back with `--dart-define=LBM_DEV=true`, which `run-live.ps1`, `run-fixtures.ps1` and `run-emulators.ps1` pass. Since 2026-09-09 `run-live.ps1` is the developer build against **production** (Grace's default while testing the cutover); `run-live.ps1 -Dev` swaps the client config to the dev project for the run and back to prod when it exits. `flutter test` always uses the demo backend.

- [x] **CP-P1 The app is production by default.** *Claude built (2026-09-08):* `LBM_DEV` flag; backend defaults to live (`LBM_BACKEND=fixtures` opts into demo); Diagnostics becomes **Staff tools**, visible in the production app only to `@the-culture-connection.com` accounts and admins (so the first admin claim can be made without a developer build; every button is still refused server-side without the claim); `.firebaserc` `prod` alias; `firebase/config/{dev,prod}/` + `scripts/use-env.ps1`; `run-prod.ps1`, `deploy-prod.ps1`, `doctor-prod.ps1`, `prod-secrets.ps1` and `npm run …:prod`; the doctor knows `prod` (the real shop is required there, the live directory expected).
- [x] **CP-P2 Production Firebase wired.** *Claude did (2026-09-08):* Android and iOS apps registered on `little-blue-cart-prod` (`flutterfire configure`); the web app `LBM admin console` registered and `admin-web/public/firebase-config.js` pointed at production (`firebase-config.dev.js` is the dev copy); all eight secrets set on prod from `PARENT\.env.littlebluemarket` by `prod-secrets.ps1` (`SHIPTURTLE_WEBHOOK_SECRET` = `unsigned`, since Shipturtle does not sign); `functions/.env.little-blue-cart-prod` written (real shop, prod client id, Shipturtle host/path, live directory, `REGISTRATION_URL` on littlebluemarket.com, `SHOPIFY_LOCATION_ID` empty until the doctor lists the real shop's locations).
- [x] **CP-P3 First production deploy.** `scripts\deploy-prod.ps1`: test-all → functions, rules, indexes, Storage rules to prod → webhooks registered on the real shop → `doctor:prod`. *Claude ran it on 2026-09-08:* rules, indexes and Storage rules landed first time; the six webhooks are registered on the real shop and already delivering (the catalog mirror has its first product); the functions needed two more passes because Google was still creating the new project's Cloud Functions and Eventarc service accounts during the first upload (a fresh-project quirk, not a code problem): **all 47 are deployed**, container cleanup policy set (3 days). Still red in the doctor for reasons that are Grace's console clicks: Anonymous sign-in off; no admin yet on prod (health check says "Admins only"); the real shop's app lacks the four write scopes.
  **Grace does, in the Firebase console for little-blue-cart-prod (once):** Blaze plan (functions need it); Authentication → Sign-in method → **Email/Password** and **Anonymous** on; Authentication → Settings → **Authorized domains** → add the Railway domain of the admin website; Project settings → Cloud Messaging → upload the APNs key (the same `.p8` as CP-N4) when the iPhone build is made; Firestore and Storage exist already (Firestore `(default)` was there; Storage is created by the first rules deploy or the console).
  **Pass:** `doctor-prod.ps1` reads 0 FAIL, with `env params … (the real shop)`, `wordpress` green (no "(live)" warning on prod), `secrets all 8 exist`, `webhooks` registered on the real shop, `auth providers` on.
  **If it fails:** a billing line → Blaze. "secret … does not exist" → `prod-secrets.ps1`. `auth providers` red → the two toggles above. `SHOPIFY_LOCATION_ID` empty → the doctor prints the real shop's locations; put the online-fulfilment one in `functions\.env.little-blue-cart-prod` and deploy again.
- [ ] **CP-P4 First run on production.** `scripts\run-prod.ps1` (or plain `flutter run`). Create your own profile with `grace-s@…` → confirm the email → Edit profile → **Staff tools** → **Claim admin** (your address must be in the prod project's `_internal/admins` document, the same CP-C0 step as on dev) → **Sync collections** → **Backfill catalog** (the real 2,500 products come in) → Edit profile → **Admin** appears.
  **Pass:** no corner badge, no error strip anywhere; a wrong password on Sign in says "That email and password do not match" and nothing more; the Market feed shows the real catalog; Edit profile shows Staff tools for you and not for a `+buyer1`-style account on another domain.
- [x] **CP-P3b Interim: production on the dev credentials (Grace's call, 2026-09-08).** Production's eight secrets were copied from the dev project's Secret Manager (`prod-secrets.ps1 -FromDev`), and `functions\.env.little-blue-cart-prod` points at the **dev test shop** (`little-blue-market-devtestingshop.myshopify.com`, its client id, location and vendor page), because those keys belong to that shop. The real shop's four lines sit commented out in the same file. The production doctor shows this as a yellow `env params` line, not red. Webhooks for the production functions are registered on the dev shop; the six on the real shop stay but are rejected (wrong signature) until CP-P6. The directory (littlebluecart.com) credentials are the same in both files, so nothing changes there.
- [ ] **CP-P6 Move production to the real shop and rotate every credential.** *Status 2026-09-08, late:* steps 1–6 done except two pieces. Production is on the **real shop** with the rotated Shopify client secret, Storefront token and littlebluecart.com credentials (seven secrets, new versions); functions redeployed; the six webhooks on the real shop confirmed; the doctor reads `env params … (the real shop)`. **Open:** (a) **Shipturtle's vendor roster does not answer for the real account.** The two keys turned out to be two Shipturtle companies: the old key is company 1092564 "little_blue_market_devtestingshop" (Grace's own login, made 2026-09-05 for the dev shop, 17 users, 8 orders); the new key is company 1066319 **"Little Blue Market"** (contact Kate & Erin, made April 2026, 1,095 orders), i.e. the real merchant account, generated by a Shipturtle-staff user named "littlebluecart". Production now runs on the real account's key. Its `/api/v1/me`, `/api/v1/orders` and `/api/v1/companies/{id}` answer in 1–3 s, but `GET /api/v1/users` (the roster automatic seller linking matches against) ends in a **504 from Shipturtle's own gateway after 120 s**, with or without paging or filter parameters; `/api/v1/vendors` is a 500. So the functions give that call 12 s and remember a failure for 10 min (`_internal/shipturtleRoster.failedAt`). **Sellers link on their own anyway, through the vendor directory (2026-09-09, Grace's ask: autonomous, nobody does anything).** Two Shipturtle calls do answer on the real account: the product list *filtered by vendor name* (a DataTables column search, ~3 s) gives the vendor's company id, and `/api/v1/companies/{id}` (~2 s) gives that company's contact email. The shop already tells the app every vendor name through Shopify's product webhooks, so: the first time a product carries a vendor name, `onCatalogWritten` resolves it (`_internal/vendorDirectory/{vendorKey}` = company id, brand, emails) and grants any verified app account behind one of those emails on the spot; the two-hourly sweep resolves whatever is still pending (about five seconds a vendor) and grants again; **Check my seller status** spends up to 70 s resolving pending vendors before it answers; the sign-in link matches against the directory in milliseconds. A vendor not found in Shipturtle's products is retried after a day, a silent Shipturtle after an hour. Nothing for Grace or the vendor to do: approval in Shipturtle puts products on the shop, the shop tells the app, the app resolves the vendor. The user-list endpoint is still tried first (12 s) and used when it answers; the slow product crawl is gone. After **Backfill catalog** on prod (all ~2,500 products, ~150 vendors) the sweep needs one or two runs to resolve everyone; to run it now: Google Cloud console → little-blue-cart-prod → Cloud Scheduler → `firebase-schedule-sellerSyncVendorRoster-us-central1` → **Force run**; `npm run logs:prod` then shows "Vendor resolved" lines and "Vendor directory sweep". **Ask Shipturtle support** (team@shipturtle.com): "GET https://api-v2.shipturtle.com/api/v1/users for company 1066319 returns 504 after 120 s; company 1092564 answers in 4 s. Is there a paged or filtered form, or a per-email lookup?" When it answers, nothing needs redeploying: the next sign-in after the 10-minute memory expires uses it. The script keeps only the token if a quote or a note surrounds it on the line; (b) the real shop's app still lacks `read_publications, write_publications, write_products, write_inventory, write_fulfillments` (step 1). **This also breaks checkout for any product not published to the app's sales channel** (found 2026-09-09: "Mobile App Development" is active with stock, the Storefront token sees 250+ other products but not it, so Buy ends in "is not available in the app's shop right now"). The backfill tries to publish every product to the channel and logs "Could not put a product on the app's channel … read_publications" for each. Two fixes, do both: grant the scopes and reinstall (then **Backfill catalog** once more publishes everything), and meanwhile in the Shopify admin: Products → select all → **…** → Add to sales channels (or Publish) → the app's channel (the Headless / custom-app channel the Storefront token belongs to), which is instant for all 14,310; (c) `SHOPIFY_LOCATION_ID` is blank; (d) the dev project still holds the OLD littlebluecart.com credentials (the note at the end of step 3). Do these in order; each is a few minutes.
  1. **New Shopify keys for the real shop.** Shopify Dev Dashboard → the Little Blue Market app → Configuration: confirm the scopes include `write_products, write_inventory, write_publications, write_fulfillments, read_customers, read_orders, read_fulfillments` → Save/Release → install (or reinstall) the app on the real shop. Client credentials → **rotate** the client secret → copy the new one into `PARENT\.env.littlebluemarket` as `SHOPIFY_CLIENT_SECRET` (and the Client ID as `SHOPIFY_CLIENT_ID` if it changed). Real shop admin → Settings → Apps and sales channels → Develop apps (or Headless) → the Storefront API token → **regenerate** → paste as `SHOPIFY_STOREFRONT_PRIVATE_TOKEN`.
  2. **New Shipturtle key.** app.shipturtle.com (your real merchant account) → Settings → API → generate a new key → paste as `SHIPTURTLE_API_KEY` in `.env.littlebluemarket` (the script also accepts `SHIPTURTLE_API_KEY_ORDER`). Delete the old key there.
  3. **New littlebluecart.com credentials.** wp-admin → Users → GraceShorter → Application Passwords → **revoke** "Little Blue Market app" → Add New with the same name → paste as `WP_APP_PASSWORD`. WooCommerce → Settings → Advanced → REST API → **revoke** the "Little Blue Market app" key → Add key (Read) → paste as `WC_CONSUMER_KEY` / `WC_CONSUMER_SECRET`. (The dev project shares these; after this step run `npm run secrets:dev -- WP_APP_PASSWORD` etc. in `functions\` for dev too, or dev's directory features stop.)
  4. **Point production at the real shop.** In `functions\.env.little-blue-cart-prod`: comment out the four interim lines, uncomment the four real-shop lines, fill `SHOPIFY_CLIENT_ID`; leave `SHOPIFY_LOCATION_ID` blank the first time.
  5. `scripts\prod-secrets.ps1` (no `-FromDev`): every value from `.env.littlebluemarket` goes into Secret Manager on prod as a new version.
  6. `scripts\deploy-prod.ps1`: redeploys the functions (they pick up the new secret versions and params), registers the six webhooks on the real shop, runs the doctor. Read the doctor: `env params … (the real shop)` green; if `SHOPIFY_LOCATION_ID` is empty it lists the real shop's locations → put the online-fulfilment one in the env file → `deploy-prod.ps1` again.
  7. **Clean up the dev shop's webhooks** that point at the production URL: `npm run webhooks:check:prod` (in `functions\`) after step 6 shows only the real shop; the dev shop's leftovers can be deleted in the dev shop admin → Settings → Notifications → Webhooks, or left (they fail signature and Shopify drops them).
  8. In the app on production: Staff tools → **Sync collections** → **Backfill catalog** (the real 2,500 products replace the test ones; test products stop being updated as soon as the dev shop's webhooks are rejected). Then Admin → **Rebuild buyer index**.
  9. **Rotate what is not a Shopify/Shipturtle/WordPress key:** Firebase console (prod) → Project settings → Service accounts → delete any downloaded service-account key (none is used); Google Cloud console → APIs & Services → Credentials → restrict the three client API keys to the Android package, the iOS bundle id and the Railway domain (after J17 passes). Railway → the admin site → Settings → regenerate the domain if it was ever shared; then Firebase Authentication → Authorized domains → replace it.
  10. Finally, delete or change the values in `PARENT\.env.dev` that were also used on production during the interim: they are the dev shop's keys, so rotating them means the same steps on the dev shop's app and Shipturtle dev account, then `npm run secrets:dev -- NAME` for each in `functions\`, then `scripts\deploy-dev.ps1`.
- [ ] **CP-P7 Push on production.** *Android needs nothing:* the production `google-services.json` is in the app; test with the production app → Edit profile → Notifications → **Allow notifications** → **Send me a test notification** → Home. *iPhone needs the APNs key, once (Grace, in two consoles):* developer.apple.com → Certificates, Identifiers & Profiles → **Keys** → + → `LBM Push` → tick **Apple Push Notifications service (APNs)** → Register → **Download** the `.p8` (one download only; keep it in `PARENT\`, never the repo), note the **Key ID**; **Team ID** from Membership. Firebase console → **little-blue-cart-prod** (not the dev project) → Project settings → **Cloud Messaging** → Apple app `com.littleblue.market` → **APNs Authentication Key** → Upload (`.p8`, Key ID, Team ID). developer.apple.com → Identifiers → `com.littleblue.market` → **Push Notifications** ticked (Xcode creates the identifier on the first signed build if it is missing). *Claude did (2026-09-09):* `ios/Runner/GoogleService-Info.plist` for production is generated and committed (dev's copy in `firebase/config/dev/`, swapped by `use-env`), so the Mac session in `Planning/mac-iphone.md` no longer needs `flutterfire configure`; it only adds the file to the Runner target in Xcode. **Pass:** the banner arrives on a locked iPhone running the production build; an announcement sent from the admin website lands on it too.
- [ ] **CP-P5 The rest of the cutover list** (unchanged from `answers-to-open-questions.md` Part 2): issue claim codes to the top five vendors (`issue-claim-code.ps1` against `--project prod`), Shipturtle webhooks pointed at the prod `shipturtleWebhook` URL, the Railway admin website's domain authorized on prod, the iPhone build on the Mac against prod (`Planning/mac-iphone.md`, `use-env prod` first). Nothing from the dev project comes across; fake orders, sellers and listings stay in `little-blue-610e5`.

### Stage 16, The branded confirmation email (Grace's ask, 2026-09-09)

**What changed.** Firebase's plain "verify your email" mail (Google's words, Google's address) is replaced by ours: the same Firebase link, minted server-side by the `sendVerificationEmail` function, sent in the app's own message (the welcome blue, the cart, the slate button) from the app's own mailbox over SMTP, and opening a page in the same style (`hosting/auth/action`, on the project's Firebase Hosting site) that says **You're confirmed**. The app's own Confirm-your-email screen flips to the same You're-confirmed card. Password-reset and email-recovery links work on that page too, so it can become the project's action URL for every Firebase email. **Nothing breaks while the mailbox is not set up:** the function says so, the app sends Firebase's plain mail instead, and the doctor shows a yellow `mail` line. The only secret is the mailbox password (`SMTP_PASS`, created as the placeholder `unset` on both projects on 2026-09-09); host, port, login and From line live in `functions\.env.<project>`.

- [ ] **CP-M1 Turn the branded mail on.** *Claude built (2026-09-09):* `functions/src/verify_email.ts` (template, renderer, throttle, SMTP delivery), the `sendVerificationEmail` callable, `hosting/` (the confirm page, the cart image, `/verified` for the stock flow), `FirebaseAuthService` calls the function and falls back to Firebase's mail, `VerifyScreen`'s confirmed state, the doctor's `mail` and `confirm page` lines, `deploy:dev`/`deploy:prod` now include hosting, `prod-secrets.ps1` knows `SMTP_PASS`.
  **Grace does, once per project (dev first, prod when it works):** (1) pick the mailbox the emails come from. **Use one on littlebluecart.com** (the domain is on Google Workspace already, with SPF, DKIM and DMARC in place, so mail from it is signed by Google and trusted by Gmail): a new `hello@littlebluecart.com` user in the Workspace admin, or your own address there. Not `littlebluecart.shop`: it has no mailbox and no reputation (that is where Firebase's plain mail comes from, and why it lands in Spam). Make it an **App Password**: myaccount.google.com → Security → 2-Step Verification (must be on) → App passwords → name it `Little Blue Market` → copy the 16 characters. (2) In `functions\.env.little-blue-610e5` fill `SMTP_HOST=smtp.gmail.com`, `SMTP_PORT=465`, `SMTP_USER=<the mailbox>`, `MAIL_FROM="Little Blue Market <the mailbox>"`. (3) In `functions\`: `npm run secrets:dev -- SMTP_PASS` and paste the App Password when asked. (4) `scripts\deploy-dev.ps1`. The doctor should now read `mail  branded confirmation email via smtp.gmail.com:465 as <the mailbox>` and `confirm page … is live`. For production: the same lines in `functions\.env.little-blue-cart-prod`, `SMTP_PASS=<the App Password>` in `PARENT\.env.littlebluemarket`, `scripts\prod-secrets.ps1 -Only SMTP_PASS`, `scripts\deploy-prod.ps1`.
  **Grace does, in the app (`run-live.ps1 -Dev`, or plain `run-live.ps1` once prod is set up):** Create a Profile with a fresh `+mail1@…` address → the Confirm-your-email screen → open the mailbox.
  **Pass:** within a minute an email from *Little Blue Market* arrives with the cart, "Confirm your email" and a grey **Confirm my email** button on blue (not Google's plain text); tapping it opens a blue page with the cart and **You're confirmed**; back in the app, within five seconds the screen becomes **You're confirmed** with the check, and **Continue** goes on to the profile setup. **Send it again** on the app screen sends a second branded mail; tapping it twice inside 30 seconds says "Give it a minute".
  **If it fails:** the mail is Google's plain one → the doctor's `mail` line is yellow: follow its fix (usually the App Password or an empty `SMTP_USER`); or `npm run logs` shows "Verification email failed … refused": a wrong App Password (regenerate), or 2-Step Verification is off on that Google account. No mail at all, not even Google's → Spam, then the dev strip's Copy for Claude. The link opens Google's plain "Your email has been verified" page instead of ours → the function on that project is old: deploy again. The page says "Firebase did not start" → paste that to Claude (the hosting site's reserved `/__/firebase/init.js` is missing).
- [ ] **CP-M2 (optional) Every Firebase email lands on our page.** Firebase console → the project → Authentication → Templates → pencil on *Email address verification* → **Customize action URL** → `https://little-blue-610e5.web.app/auth/action` (prod: `https://little-blue-cart-prod.web.app/auth/action`) → Save. This applies to password resets too. **Pass:** Sign in → Forgot password → the reset email's link opens our blue **Choose a new password** page; saving one and signing in with it works. Until this is done the branded confirmation mail already uses our page (the function rewrites the link); only Firebase's own emails still show Google's page, which then continues to our `/verified`.
- [ ] **CP-M3 Keep it out of Spam.** *Found 2026-09-09:* the first confirmation mail (Firebase's plain one, `noreply@littlebluecart.shop`) landed in Gmail's Spam: "similar to messages identified as spam in the past". Three causes, in order of weight: a generic text-only message with a raw `firebaseapp.com` link; a sender domain with no mailbox, no history and **no DMARC record** (`littlebluecart.shop` has Firebase's SPF and DKIM, nothing else); and a link domain that is not the sender's. CP-M1 fixes the first two at once (our message, sent through Google Workspace from littlebluecart.com, which has SPF, DKIM and DMARC). The rest, each a few minutes at GoDaddy (DNS Management, the same screen as `PARENT\DNS records.pdf`, domain **littlebluecart.shop**):
  1. **DMARC on littlebluecart.shop** (keeps Firebase's own mails, password resets included, from scoring as unauthenticated): Add New Record → Type `TXT`, Name `_dmarc`, Data `v=DMARC1; p=none; rua=mailto:grace-s@the-culture-connection.com`, TTL 1 hour → Save. Gmail asks for exactly this from every sender since 2024. (`littlebluecart.com` already has `v=DMARC1; p=none;`.)
  2. **The confirm page on your own domain**, so the link in the mail matches the sender: Firebase console → **little-blue-cart-prod** → Hosting → **Add custom domain** → `app.littlebluecart.shop` → it shows one or two records (a TXT for ownership, then an A or CNAME) → add them at GoDaddy → wait for the green tick (minutes to an hour). Then Authentication → Settings → **Authorized domains** → add `app.littlebluecart.shop`. Then in `functions\.env.little-blue-cart-prod` set `PUBLIC_WEB_URL=https://app.littlebluecart.shop` and run `scripts\deploy-prod.ps1`. (Dev keeps `little-blue-610e5.web.app`; nobody but us sees dev mail.)
  3. In the inbox that got the first one, tap **Report not spam** once; Gmail learns per mailbox.
  4. **(Grace started 2026-09-09) littlebluecart.com as Firebase's sender domain**, so Firebase's own mails come from `noreply@littlebluecart.com` instead of the .shop. Firebase shows four records. Three are new and touch nothing (add them first, any time): TXT `@` = `firebase=little-blue-cart-prod`; CNAME `firebase1._domainkey` = `mail-littlebluecart-com.dkim1._domainkey.firebasemail.com`; CNAME `firebase2._domainkey` = `mail-littlebluecart-com.dkim2._domainkey.firebasemail.com`. The fourth is an **edit** of the existing SPF TXT at `@`, never a second SPF record. Old line (the rollback, exact): `v=spf1 include:dc-aa8e722993._spfm.littlebluecart.com include:_spf.google.com ~all`. New line: `v=spf1 include:dc-aa8e722993._spfm.littlebluecart.com include:_spf.google.com include:_spf.firebasemail.com ~all`. Six DNS lookups of the ten allowed after the change (checked 2026-09-09). Reversible by pasting the old line back; `~all` means a mistake softens delivery, never rejects it. Claude checks with `nslookup -type=TXT littlebluecart.com 8.8.8.8` after each step.
  **Pass:** a fresh `+mail2@…` sign-up on production gets the branded mail in the Inbox, not Spam; in Gmail → the message's ⋮ menu → *Show original* the three lines read `SPF: PASS`, `DKIM: PASS`, `DMARC: PASS`; the button's link starts with `https://app.littlebluecart.shop/auth/action`.
  **If it fails:** still Spam with all three PASS → it is the mailbox's newness; send a handful of ordinary mails from it first (Gmail warms a new address up over a week), and never send from it to addresses that do not exist. `DKIM: FAIL` → the App Password is on a different account than `SMTP_USER`, or `MAIL_FROM` is not the same domain as `SMTP_USER` (Google rewrites the From and Gmail then treats it as forged); make them the same mailbox.

### Stage 17, The six changes (Grace's asks, 2026-09-14)

**What this is.** Grace's list of six, planned in `Planning/stage-17-disclaimer-directory-promos.md`, which is the engineering detail behind these boxes. Four decisions she made up front: adverts and announcements are uploaded from the **admin website**, not the app; the disclaimer's second line is about **sellers and the bio on their profile**; a popup shows **once per app opening** and never the same one twice; **unclaimed directory listings stay out of the Market feed** and live under Browse the directory and in search. Order matters only in that CP-17A comes before CP-17B, because the disclaimer tells people to tap the three dots.

- [ ] **CP-17A The three dots report a post.** *Claude built:* `lib/widgets/post_card.dart` `_PostHead` becomes a `ConsumerWidget` and its "…" calls the `showMoreSheet` that the post screen and seller profiles already use. Nothing new: the sheet, the reasons, the send and the Admin screen's reports list were all built in Stage 13.
  **Grace does:** Market feed → the "…" at the top right of any post that is not hers → **Report this post** → a reason → **Send report**. Then, signed out or as a guest, the same tap. Then the "…" on one of her own posts.
  **Pass:** the sheet opens with Report this post; sending it says "Reported. Thank you; Little Blue Market will look."; the report appears under You → Admin → Reports with the post it came from. As a guest the tap asks her to make a profile first. On her own post the row is greyed with "This is you."
  **If it fails:** nothing happens on the tap → the old build is still on the phone, run `run-live.ps1` again. "Missing or insufficient permissions" → paste the dev strip's Copy for Claude.

- [ ] **CP-17B The disclaimer after the tutorial.** *Claude built:* `lib/widgets/keep_it_here_dialog.dart`, shown by `app_shell.dart` the moment the first-time tour closes, by Done or by Skip. One **I understand** button, no tapping outside to dismiss, remembered per phone by the same `Tips.firstTour` flag the tour uses.
  **Grace does:** Create a Profile with a fresh `+dis1@…` address, finish setup, and let the tour appear. Tap through it to **Done**. Then, on a second fresh account, tap **Skip** on the first page.
  **Pass:** both ways, the blue card appears straight after the tour: "Keeping Little Blue Cart alive", the two paragraphs about keeping transactions on the platform and reporting anyone who pushes a purchase elsewhere, the line asking sellers to put their Little Blue Cart link in their profile bio, and the line about the three dots. **I understand** closes it. Closing and reopening the app does not show it again.
  **If it fails:** it never appears → the phone already remembers the tour from an earlier account; a fresh account or a reinstall is the test. It appears every launch → `markSeen` did not write; paste Copy for Claude.

- [ ] **CP-17C Post an advert from the admin website.** *Claude built:* `promos/{id}` (rules: public read when active, writes only from a callable), `promos/` in Storage (public read, admin-claim write), `functions/src/promos.ts` with `adminPromoSave`, `adminPromoSetActive` and `adminPromoDelete`, and a third card on the admin website: title, caption, photos, the CTA button's wording, the link, the audience, a live preview of the popup, and the list of what is live with Pause and Delete on each.
  **Grace does:** open the Railway admin website, sign in, scroll to **Adverts and announcements** → kind **Advert** → a title, a caption, pick a photo, CTA wording ("Shop the sale"), a link, audience **Everyone** → **Post it**.
  **Pass:** the preview shows the card as the phone will draw it; after posting, the advert appears in the list below as Live with 0 seen and 0 tapped.
  **If it fails:** "Admins only" → the account has no admin claim (app → Edit profile → Diagnostics → Claim admin, then sign in here again). The photo never finishes uploading → the Storage rules are not deployed: `scripts\deploy-dev.ps1`.

- [ ] **CP-17D The popups fade in on the phone.** *Claude built:* `lib/widgets/promo_popup.dart`, hosted by `app_shell` so it floats over any tab. Fades in over 420 ms about three seconds after the feed settles as a **modal**: the app behind greyed down with the brand ink, the card centred, the picture a 4:5 poster at the top (a swipeable carousel with dots when there is more than one), then who it is from, the title, the caption and the button, with the X on the dimmed app above the card. It fades out on the X, on the button, on a tap anywhere outside, on a flick downwards, or on its own after twelve seconds. **This replaced a quieter bottom card on 2026-09-14** at Grace's request for something "a bit more dramatic"; the trade she accepted is that the feed behind it does not scroll while it is up. Once per app opening, never the same one twice (seen ids beside `Tips`). The CTA opens the link in the browser. Seen and tapped are counted with `FieldValue.increment`. Off under `FLUTTER_TEST`. The announcement form now carries a photo, a CTA and a link too, and an announcement still pushes and still shows under the bell exactly as before, and now also fades in.
  **Grace does:** with the advert from CP-17C live, force-close the app and open it. Wait on the Market feed. Then tap the CTA. Then force-close and open again. Then post an **Announcement** from the website with a photo and a button, and open the app again.
  **Pass:** the advert fades in gently after a few seconds over the feed, the screen behind it still scrolls and its buttons still work, the X closes it with a fade and so does a flick downwards; the CTA opens the link in the browser; the second opening shows nothing, because that one has been seen; the announcement fades in the same way and is also a row under the bell (the bell row stays words, as it always was; the photo and the button belong to the popup). The admin website's list now reads 1 seen, 1 tapped.
  **Testing them without reinstalling (Grace's ask, 2026-09-14):** on a developer build, and so on the staging website, **Edit profile → Diagnostics → Adverts and announcements** lists everything live, whoever it is aimed at, and says of each one whether it is aimed at you and whether this phone has seen it. **Show it** puts any of them on screen at once, ignoring the three-second wait, the one-per-opening rule, the never-twice rule and the audience, and counting nothing (an impression logged while testing would be a lie in the advert's own numbers). **Forget what I have seen** and **Let another one show** put the normal flow back to the start.
  **If it fails:** nothing ever fades in → the promo is not active, or the audience excludes her (Sellers when she is not one). It appears every single launch → the seen ids did not save; paste Copy for Claude. It blocks the feed → report it, the barrier is meant to be transparent.

- [ ] **CP-17E Pull the whole directory in.** *Claude built:* `syncPublicDirectory` in `functions/src/directory.ts`, which pages through every **published** listing on littlebluecart.com (the crawl that already builds the owner index, without the credential, so published only), resolves the category, tag and location names through the cache that is already there, and mirrors them all into `directoryListings/{wpPostId}` with `categorySlugs`, `titleWords`, `titleLower` and `unclaimed`. It never blanks an `ownerUid` that is already set and it never writes a feed post. It rolls up `directoryCategories/{slug}` with a count. Wired into the six-hourly `directorySyncScheduled`, plus an `adminSyncDirectory` callable and a **Pull the directory now** button on the admin website that reports the count.
  **Grace does:** admin website → **Pull the directory now**. Wait for the number.
  **Pass:** it reports how many listings and how many categories it found, and the number of listings is roughly what littlebluecart.com shows when browsing the directory logged out. Tapping it again a minute later is safe and reports the same numbers.
  **If it fails:** it reports 0 → the dev project points at staging and staging's directory is empty; that is expected on dev, so this checkpoint is the one to run against **production** (`scripts\deploy-prod.ps1` first). A timeout → the crawl is bigger than one call allows; paste the number it reached and Claude splits it.

- [ ] **CP-17F Browse and search the directory, and claim from it.** *Claude built:* a **Browse the directory** rail on the Market feed under Browse the shop, one chip per directory category in the site's own words (hidden entirely until the pull has run, the same way Browse the shop hides); a category screen drawn with the `DirectoryListingCard` that already exists, a page at a time with **Show more**, because nobody knows yet how many listings the site has; directory results in the search screen under their own heading, **below** the shop's results and outside the shop's "nothing found" card (a word that matches only a directory business used to say "Nothing for …" and hide the business it had just found); and on every unclaimed listing, **Is this your business?**, which goes to the directory link screen Grace has already tapped through.
  **Grace does:** Market feed → under Browse the shop, **Browse the directory** → tap a category → read a listing, tap Website and Directions on it. Then the search pill → the name of a business that is in the directory but has no app account. Then, on an unclaimed listing, **Is this your business?** Then, signed in as the `+dir1@…` account whose email matches a real listing, follow that flow through.
...
**Functions:** `functions/src/{index,config,linking,sellers,vendors,catalog,orders,cart,fulfillment,shipturtle}.ts` · `functions/src/shopify/{token,storefront}.ts` · **Stages 10–12:** `functions/src/{wordpress,directory,push}.ts` · `functions/scripts/wp-probe.mjs` · `functions/src/{directory_products,buyer_index,notifications}.ts` · `admin-web/` (the Railway announcement site; static, no server logic) · **Stage 16:** `functions/src/verify_email.ts` · `hosting/` (the confirm page and the email's cart image, on Firebase Hosting) · `functions/scripts/{register-webhooks,seed}.mjs` · `functions/test/*` · `firebase/{firestore.rules,storage.rules,firestore.indexes.json}` · `.firebaserc` · `functions/.env.little-blue-610e5`.
...
**Manual, per checkpoint:** the "Grace does / Pass looks like" lines above, on the Android emulator via `scripts\run-live.ps1` (production data with the developer surfaces on; `-Dev` for the dev project and test shop). The groups S, A, C, P, R, M in `implementation-phases.md` map onto Stages 1–7 and stay the detailed reference.

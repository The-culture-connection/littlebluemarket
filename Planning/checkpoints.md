...
- [ ] **CP-F2 The splash screen.** *Claude built (2026-09-08):* Grace's `Body.png` is `assets/images/splash-body.png`. The native launch screens on both phones are now its exact blue (`#70A0D1`: Android `launch_background.xml` + Android 12's system splash colour, iOS `LaunchScreen.storyboard`), and the app holds the full artwork over its first frame for a moment before fading to the welcome screen. No plugin; one image, one colour. **App icon (added the same evening):** Grace's `Body (1).png` is `assets/icon/app-icon.png`; `dart run flutter_launcher_icons` writes every Android and iOS size, with an adaptive icon (blue background, the cart padded into the safe zone in `assets/icon/app-icon-fg.png`). Regenerate with that command after changing the image; then `git checkout -- ios/Runner.xcodeproj/project.pbxproj`, because the generator edits the wrong Xcode setting.

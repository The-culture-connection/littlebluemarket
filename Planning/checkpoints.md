...
- [ ] **CP-P6 Move production to the real shop and rotate every credential.** *Status 2026-09-08, late:* steps 1–6 done except two pieces. Production is on the **real shop** with the rotated Shopify client secret, Storefront token and littlebluecart.com credentials (seven secrets, new versions); functions redeployed; the six webhooks on the real shop confirmed; the doctor reads `env params … (the real shop)`. **Open:** (a) the Shipturtle Order key in `.env.littlebluemarket` is corrupted (a `€` around character 1018, the same as the dev copy), so production keeps the previous working Order key until a clean paste, then `prod-secrets.ps1 -Only SHIPTURTLE_API_KEY` + `deploy-prod.ps1`; (b) the real shop's app still lacks `write_products, write_inventory, write_publications, write_fulfillments` (step 1), so sellers cannot add products on production yet; (c) `SHOPIFY_LOCATION_ID` is blank; (d) the dev project still holds the OLD littlebluecart.com credentials (the note at the end of step 3). Do these in order; each is a few minutes.
